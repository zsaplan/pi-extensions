...
export function composeResponseReviewPrompt(
  ...
  const includeOriginalResponse = comments.length === 0 || comments.some((comment) => comment.excerpt.trim().length === 0);

  lines.push("Review feedback was captured for the assistant response below.");
  lines.push("Use the feedback contextually: discuss it further, make code changes, record memory, use tools, ask follow-up questions, or revise the response itself as appropriate.");
  lines.push("");
  ...
  if (comments.length === 0) {
    lines.push("- No line-specific comments were added. Apply the overall review note only.");
  } else {
  ...
  lines.push("Address the feedback in the most appropriate way for the current task.");

...
function isSubmitPayload(value: ResponseReviewWindowMessage): value is ResponseReviewSubmitPayload {
  return value.type === "submit";
}

function isCancelPayload(value: ResponseReviewWindowMessage): value is ResponseReviewCancelPayload {
  return value.type === "cancel";
}

function isRequestPayload(value: ResponseReviewWindowMessage): value is ResponseReviewRequestPayload {
  return value.type === "request-response";
}

function isClipboardWritePayload(value: ResponseReviewWindowMessage): value is ResponseReviewClipboardWritePayload {
  return value.type === "clipboard-write";
}

function isClipboardReadPayload(value: ResponseReviewWindowMessage): value is ResponseReviewClipboardReadPayload {
  return value.type === "clipboard-read";
}

type WaitingEditorResult = "escape" | "window-settled";

async function runClipboardCommand(command: string, args: string[], options?: { input?: string }): Promise<{ stdout: string; stderr: string; code: number | null }> {
...
async function writeSystemClipboard(text: string): Promise<void> {
  ...
  if (process.platform === "linux") {
    const wlCopy = await runClipboardCommand("bash", ["-lc", "command -v wl-copy >/dev/null 2>&1 && wl-copy || command -v xclip >/dev/null 2>&1 && xclip -selection clipboard || command -v xsel >/dev/null 2>&1 && xsel --clipboard --input || exit 127"], { input: text });
    if (wlCopy.code === 0) return;
    throw new Error(wlCopy.stderr.trim() || "No clipboard writer available");
  }

  if (process.platform === "win32") {
    const result = await runClipboardCommand("powershell", ["-NoProfile", "-Command", "Set-Clipboard -Value ([Console]::In.ReadToEnd())"], { input: text });
    if (result.code === 0) return;
...
async function readSystemClipboard(): Promise<string> {
  ...
  if (process.platform === "linux") {
    const result = await runClipboardCommand("bash", ["-lc", "command -v wl-paste >/dev/null 2>&1 && wl-paste -n || command -v xclip >/dev/null 2>&1 && xclip -selection clipboard -o || command -v xsel >/dev/null 2>&1 && xsel --clipboard --output || exit 127"]);
    if (result.code === 0) return result.stdout;
    throw new Error(result.stderr.trim() || "No clipboard reader available");
  }

  if (process.platform === "win32") {
    const result = await runClipboardCommand("powershell", ["-NoProfile", "-Command", "Get-Clipboard -Raw"]);
    if (result.code === 0) return result.stdout;
...
export default function (pi: ExtensionAPI) {
  ...
  async function reviewResponses(rawArgs: string, ctx: ExtensionCommandContext): Promise<void> {
    ...
    if (loadedSession.responses.length === 0) {
      ctx.ui.notify("No assistant responses with visible text were found.", "info");
      return;
    }

    const responseMap = new Map<string, ResponseReviewEntryData>(loadedSession.responses.map((response) => [response.id, response]));
    const windowData: ResponseReviewWindowData = {
      session: loadedSession.session,
      responses: loadedSession.responses.map(({ text: _text, precedingUserText: _precedingUserText, ...summary }) => summary),
    };
    ...
    const sendHostDebug = (message: string, details?: Record<string, unknown>): void => {
      sendWindowMessage({
        type: "debug-log",
        source: "host",
        message,
    ...
    try {
      const terminalMessagePromise = new Promise<ResponseReviewSubmitPayload | ResponseReviewCancelPayload | null>((resolve, reject) => {
        let settled = false;
        ...
        const settle = (value: ResponseReviewSubmitPayload | ResponseReviewCancelPayload | null): void => {
          if (settled) return;
          settled = true;
          cleanup();
          resolve(value);
        ...
        const handleRequest = async (message: ResponseReviewRequestPayload): Promise<void> => {
        ...
        const handleClipboardWrite = async (message: ResponseReviewClipboardWritePayload): Promise<void> => {
          ...
          try {
            ...
            sendHostDebug("clipboard-write success", { requestId: message.requestId });
            sendWindowMessage({
        ...
        const handleClipboardRead = async (message: ResponseReviewClipboardReadPayload): Promise<void> => {
          sendHostDebug("clipboard-read received", { requestId: message.requestId });
          try {
      ...
      const response = responseMap.get(message.responseId);
      if (response == null) {
        ctx.ui.notify("Response review failed: selected response was not found.", "error");
        return;
      }

      const prompt = composeResponseReviewPrompt(response, message);
      ctx.ui.setEditorText(prompt);
      ctx.ui.notify("Inserted response review feedback into the editor.", "info");
    } catch (error) {
  ...
  pi.registerCommand("response-review", {
    description: "Open a native review window for assistant responses in the current session or a retroactive session",

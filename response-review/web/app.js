const reviewData = JSON.parse(document.getElementById("response-review-data").textContent || "{}");

const allResponses = [...(reviewData.responses || [])].sort((a, b) => b.index - a.index);
const responseById = new Map(allResponses.map((response) => [response.id, response]));
...
const currentResponseLabelEl = document.getElementById("current-response-label");
const responseMetaEl = document.getElementById("response-meta");
const modeHintEl = document.getElementById("mode-hint");
const editorContainerEl = document.getElementById("editor-container");
const responseCommentsContainer = document.getElementById("response-comments-container");
...
sessionLabelEl.textContent = reviewData.session?.sessionPath || reviewData.session?.displayTitle || "";

document.getElementById("window-title").textContent = `Response review · ${reviewData.session?.displayTitle || "session"}`;
...
let pendingHostRequests = new Map();
...
function scoreSubsequence(query, candidate) {
  ...
  for (let i = 0; i < candidate.length && queryIndex < query.length; i += 1) {
    ...
    if (i === 0 || previousChar === " " || previousChar === "/" || previousChar === "#" || previousChar === ":") {
      score += 12;
    }
...
function getFilteredResponses() {
  ...
  return allResponses
    .map((response) => ({ response, score: getResponseSearchScore(query, response) }))
...
function activeResponse() {
  return state.activeResponseId ? responseById.get(state.activeResponseId) || null : null;
}
...
function totalCommentCount() {
  return Object.values(state.drafts).reduce((sum, draft) => sum + (draft?.comments?.length || 0), 0);
}
...
function modelLabel(response) {
  if (!response) return "";
  if (response.provider && response.model) return `${response.provider}/${response.model}`;
...
function ensureActiveResponse() {
  if (state.activeResponseId && responseById.has(state.activeResponseId)) return;
...
function getActiveCommentTarget() {
  ...
  if (selectionRange != null) {
    const existingComment = findInlineCommentOverlappingRange(selectionRange.startLine, selectionRange.endLine);
    if (existingComment) {
  ...
  const lineNumber = Math.max(1, selection.positionLineNumber ?? selection.endLineNumber);
...
function normalizeEditorSelectionToWholeLines() {
  if (!editor || !model || !monacoApi || isNormalizingEditorSelection) return false;
  const selection = editor.getSelection();
...
function ensureSelectionWidget() {
  ...
  domNode.type = "button";
  domNode.className = "inline-flex h-4 w-4 items-center justify-center rounded-[4px] border border-[rgba(240,246,252,0.08)] bg-[#238636] p-0 text-[12px] font-semibold leading-none text-white shadow-lg hover:bg-[#2ea043]";
...
function updateSelectionWidget() {
  ...
  if (visible) {
    domNode.textContent = "+";
    domNode.title = target.mode === "existing"
...
function updateButtons() {
  ...
  const draft = activeDraft();
  overallCommentButton.textContent = draft?.overallComment?.trim() ? "Overall note ✓" : "Overall note";
...
function renderSidebar() {
  ...
  if (visible.length === 0) {
    ...
    visible.forEach((response) => {
      ...
      const pending = state.pendingRequestIds[response.id] != null && state.responseContents[response.id] == null;
      const errored = state.responseErrors[response.id] != null;
  ...
  currentResponseLabelEl.textContent = active ? `Response #${active.index}` : "No response selected";
  responseMetaEl.textContent = activeMetaText(active);
  const commentTarget = active != null && isActiveResponseReady() ? getActiveCommentTarget() : null;
...
function ensureResponseLoaded(responseId) {
  ...
  if (current.text != null || current.error != null || current.requestId != null) return;
...
function showConfirmModal(options) {
  const confirmClassName = options.confirmTone === "danger"
...
function showOverallCommentModal() {
  ...
  showTextModal({
    ...
    description: "This note is included above the line-targeted review comments in the generated prompt.",
...
function normalizeInlineCommentRange(startLine, endLine) {
  ...
  const safeEndLine = Math.max(safeStartLine, Math.max(startLine, endLine || startLine));
  return {
...
function getInlineComments(draft = activeDraft()) {
  if (!draft) return [];
  return draft.comments.filter((comment) => comment.startLine != null);
}
...
function findInlineCommentCoveringLine(lineNumber, draft = activeDraft()) {
  return getInlineComments(draft).find((comment) => {
    const endLine = inlineCommentLineEnd(comment);
    return lineNumber >= comment.startLine && lineNumber <= endLine;
  }) || null;
}

function findInlineCommentOverlappingRange(startLine, endLine, draft = activeDraft()) {
  const normalizedRange = normalizeInlineCommentRange(startLine, endLine);
  return getInlineComments(draft).find((comment) => lineRangesOverlap(
...
function findCommentTextarea(commentId) {
  return [...document.querySelectorAll("textarea[data-comment-id]")].find((textarea) => textarea.getAttribute("data-comment-id") === commentId) || null;
}
...
function focusCommentTextarea(textarea, options = {}) {
  ...
  const start = Math.max(0, Math.min(Number.isFinite(options.start) ? Number(options.start) : maxOffset, maxOffset));
  const end = Math.max(start, Math.min(Number.isFinite(options.end) ? Number(options.end) : start, maxOffset));
  textarea.setSelectionRange(start, end, options.direction || "none");
  logTextareaSelection(textarea, "comment textarea focused", { start, end, direction: options.direction || "none" });
}
...
function scheduleFocusCommentInput(commentId, options = {}) {
  const maxAttempts = Number.isFinite(options.maxAttempts) ? Number(options.maxAttempts) : 12;
  let attempts = 0;
...
function addInlineComment(startLine, endLine) {
  ...
  const normalizedRange = normalizeInlineCommentRange(startLine, endLine);
  const existingComment = findInlineCommentOverlappingRange(normalizedRange.startLine, normalizedRange.endLine, draft);
  if (existingComment) {
  ...
  const comment = {
    ...
    excerpt: getExcerptForRange(text, normalizedRange.startLine, normalizedRange.endLine),
    body: "",
  };
  draft.comments.push(comment);
  renderAll({ preserveScroll: true });
  scheduleFocusCommentInput(comment.id, { revealLineNumber: inlineCommentLineEnd(comment) });
}

function showResponseCommentModal() {
  const response = activeResponse();
  const draft = activeDraft();
  if (!response || !draft) return;
  showTextModal({
    title: `Whole-response comment for response #${response.index}`,
    description: "This comment applies to the entire assistant response.",
    initialValue: "",
...
function restoreEditorSelectionSnapshot(snapshot) {
  ...
  editor.setPosition({ lineNumber: selection.positionLineNumber, column: selection.positionColumn });
...
function maybeOpenCommentForActiveEditorLine(options = {}) {
  if (!editor) return;
  const lineNumber = editor.getPosition()?.lineNumber ?? editor.getSelection()?.positionLineNumber ?? null;
...
function collapseComment(comment, options = {}) {
  const nextLine = Math.max(comment.startLine ?? 1, (comment.endLine ?? comment.startLine ?? 1) + 1);
  const targetLine = Number.isFinite(options.targetLine) ? Number(options.targetLine) : nextLine;
...
function wireCommentInput(element) {
  ...
  ["mousedown", "mouseup", "click", "dblclick", "pointerdown", "wheel"].forEach((eventName) => {
...
function installTextareaShortcutFallback(textarea) {
  ...
  const flushPendingClipboardAction = (reason, options = {}) => {
    ...
    if ((modifierState.meta || modifierState.ctrl) && !allowWhileModifierHeld) {
      ...
      pendingFlushTimer = window.setTimeout(() => flushPendingClipboardAction("timer"), 75);
      return;
    }
    ...
    const timeoutMs = Number.isFinite(options.timeoutMs) ? Number(options.timeoutMs) : (allowWhileModifierHeld ? 300 : 2000);
    ...
    if (action.kind === "write") {
      ...
      void sendHostRequest({
        ...
        debugLog("textarea host clipboard write result", { key: action.key, requestId, result });
        if (result?.ok) {
          pendingClipboardAction = null;
          if (action.key === "x") {
            textarea.setRangeText("", action.selectionStart, action.selectionEnd, "start");
            dispatchTextareaInput(textarea);
          }
          return;
        }
        if (allowWhileModifierHeld && !action.didRetryAfterModifierRelease && !(modifierState.meta || modifierState.ctrl)) {
          action.didRetryAfterModifierRelease = true;
          debugLog("textarea host clipboard write retrying after modifier release", { key: action.key });
          flushPendingClipboardAction("retry-after-modifier-release");
          return;
        }
        if (!allowWhileModifierHeld || !(modifierState.meta || modifierState.ctrl)) {
          debugLog("textarea host clipboard write abandoned", { key: action.key, requestId });
          pendingClipboardAction = null;
        }
      });
      return;
    ...
    void sendHostRequest({
      ...
      if (result?.ok && typeof result.text === "string") {
        pendingClipboardAction = null;
        textarea.setRangeText(result.text, action.selectionStart, action.selectionEnd, "end");
        dispatchTextareaInput(textarea);
        return;
      }
      if (allowWhileModifierHeld && !action.didRetryAfterModifierRelease && !(modifierState.meta || modifierState.ctrl)) {
        action.didRetryAfterModifierRelease = true;
        debugLog("textarea host clipboard read retrying after modifier release", {});
        flushPendingClipboardAction("retry-after-modifier-release");
        return;
      }
      if (!allowWhileModifierHeld || !(modifierState.meta || modifierState.ctrl)) {
  ...
  const queueClipboardAction = (action) => {
    ...
    pendingFlushTimer = window.setTimeout(() => {
      flushPendingClipboardAction("initial-timer-force", { allowWhileModifierHeld: true, timeoutMs: 300 });
    }, 125);
  };
  ...
  const handleShortcutKeyup = (event, source) => {
    ...
    const key = String(event.key || "").toLowerCase();
    debugLog(`${source} modifier keyup:${key}`, { metaKey: event.metaKey, ctrlKey: event.ctrlKey });
    if (!pendingClipboardAction) return;
    if (key === pendingClipboardAction.key) {
      flushPendingClipboardAction(`${source}-keyup-action-key`, { allowWhileModifierHeld: true, timeoutMs: 300 });
      return;
    }
  ...
  textarea.addEventListener("blur", () => {
    ...
    debugLog("textarea blur", { activeTag: document.activeElement?.tagName ?? null });
  ...
  textarea.addEventListener("keydown", (event) => {
    ...
    if (key === "c" || key === "x") {
      ...
      if (selectedText.length === 0) {
        debugLog("textarea host copy/cut skipped", { key, reason: "empty selection" });
        return;
      }

      event.preventDefault();
      event.stopPropagation();
      debugLog("textarea host clipboard write queued", { key, selectionLength: selectedText.length });
    ...
    if (key === "v") {
      ...
      const selectionStart = textarea.selectionStart ?? 0;
      const selectionEnd = textarea.selectionEnd ?? selectionStart;
      debugLog("textarea host clipboard read queued", { selectionStart, selectionEnd });
      queueClipboardAction({
        kind: "read",
        key,
        selectionStart,
...
function autosizeCommentTextarea(textarea) {
  ...
  const previousHeight = Number.parseFloat(textarea.style.height || "0") || textarea.getBoundingClientRect().height;
  textarea.style.height = "auto";
  const contentLines = Math.max(1, Math.ceil((textarea.scrollHeight - verticalPadding) / lineHeight));
...
function renderCommentDOM(comment, onDelete, onLayoutChange = () => {}) {
  ...
  const toggleButton = container.querySelector("[data-action='toggle-collapse']");
  ...
  if (textarea != null) {
    ...
    textarea.addEventListener("keydown", (event) => {
      const isPlainArrowNavigation = !event.metaKey && !event.ctrlKey && !event.altKey && !event.shiftKey;
      const selectionStart = textarea.selectionStart ?? 0;
      const selectionEnd = textarea.selectionEnd ?? selectionStart;
      const textLength = textarea.value.length;

      if (comment.startLine != null && isPlainArrowNavigation && event.key === "ArrowUp" && selectionStart === 0 && selectionEnd === 0) {
        event.preventDefault();
        event.stopPropagation();
      ...
      if (comment.startLine != null && isPlainArrowNavigation && event.key === "ArrowDown" && selectionStart === textLength && selectionEnd === textLength) {
        event.preventDefault();
        event.stopPropagation();
      ...
      if (event.key === "Escape") {
        ...
        if (textarea.value.trim().length > 0) {
          ...
          showConfirmModal({
            ...
            confirmTone: "danger",
            onConfirm: () => {
              deleteComment(comment, comment.startLine != null ? { focusLine: comment.startLine } : {});
            },
            onCancel: () => {
              requestAnimationFrame(() => {
      ...
      const isCollapseShortcut = (event.metaKey || event.ctrlKey) && event.key === "Enter";
...
function syncViewZones() {
  ...
  const draft = activeDraft();
  if (!draft) return;
  const inlineComments = draft.comments.filter((comment) => comment.startLine != null);
...
function updateDecorations() {
  ...
  const draft = activeDraft();
  const inlineComments = draft ? draft.comments.filter((comment) => comment.startLine != null) : [];
  const nextDecorations = inlineComments.map((comment) => ({
    range: new monacoApi.Range(comment.startLine, 1, comment.endLine || comment.startLine, 1),
    options: {
      isWholeLine: true,
...
function renderWholeResponseComments() {
  ...
  const comments = draft.comments.filter((comment) => comment.startLine == null);
  if (comments.length === 0) {
    responseCommentsContainer.className = "hidden overflow-hidden px-0 py-0";
    return;
  }

  responseCommentsContainer.className = "space-y-4 border-b border-review-border bg-[#0d1117] px-4 py-4";
...
function isActiveResponseReady() {
  ...
  const request = requestState(response.id);
  return request.text != null && request.error == null;
}
...
function buildSubmitPayload() {
  ...
  if (payload.overallComment.length === 0 && payload.comments.length === 0) return null;
  return payload;
}
...
function getMouseClickCount(event) {
  return event?.event?.detail
    ?? event?.event?.browserEvent?.detail
    ?? 1;
}
...
function createGlyphHoverActions() {
  ...
  editor.onMouseDown((event) => {
    ...
    if (clickCount >= 2 && line != null) {
      clearPendingGutterClick();
      const lineEndColumn = model != null ? model.getLineMaxColumn(line) : 1;
      setTimeout(() => {
    ...
    if (target.type === monacoApi.editor.MouseTargetType.GUTTER_GLYPH_MARGIN || target.type === monacoApi.editor.MouseTargetType.GUTTER_LINE_NUMBERS) {
      ...
      pendingGutterClickTimer = setTimeout(() => {
        pendingGutterClickTimer = null;
        const selectionRange = getSelectionLineRange();
        if (selectionRange != null && line >= selectionRange.startLine && line <= selectionRange.endLine) {
...
window.__responseReviewReceive = function (message) {
  ...
  if (typeof message.requestId === "string" && pendingHostRequests.has(message.requestId)) {
  ...
  if (message.type === "response-error") {
    state.responseErrors[message.responseId] = message.message || "Unknown error";
    delete state.pendingRequestIds[message.responseId];
    renderSidebar();
    if (state.activeResponseId === message.responseId) {
...
function setupMonaco() {
  ...
  window.require(["vs/editor/editor.main"], function () {
    ...
    editor.onDidFocusEditorText(() => {
      debugLog("editor focus", { line: editor.getPosition()?.lineNumber ?? null });
    });
    editor.onDidBlurEditorText(() => {
      shouldOpenCommentOnArrowNavigation = false;
      debugLog("editor blur", { activeTag: document.activeElement?.tagName ?? null });
    });
    editor.onKeyDown((event) => {
      if (!event.metaKey && !event.ctrlKey && !event.altKey && [monacoApi.KeyCode.UpArrow, monacoApi.KeyCode.DownArrow].includes(event.keyCode)) {
        shouldOpenCommentOnArrowNavigation = true;
      }
      if ((event.metaKey || event.ctrlKey) && !event.altKey && event.keyCode === monacoApi.KeyCode.Enter) {
        event.preventDefault();
        event.stopPropagation();
      ...
      if (event.keyCode !== monacoApi.KeyCode.Enter || event.metaKey || event.ctrlKey || event.altKey) {
        return;
      }
      const target = getActiveCommentTarget();
      if (target == null) return;

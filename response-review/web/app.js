...
function renderDebugPanel() {
    ensureDebugPanel();
    if (debugState.panel === null || debugState.pre === null)
        return;
...
function hasActiveFeedback() {
    ...
    if (!draft)
        return false;
    if (draft.overallComment.trim().length > 0)
        return true;
...
function getActiveCommentTarget() {
    if (!editor || !model || !isActiveResponseReady())
        return null;
    const selection = editor.getSelection();
    if (!selection)
        return null;
...
function normalizeEditorSelectionToWholeLines() {
    ...
    if (!selectionRange)
        return false;
    const normalizedSelection = new monacoApi.Selection(selectionRange.startLine, 1, selectionRange.endLine, model.getLineMaxColumn(selectionRange.endLine));
    if (selection.startLineNumber === normalizedSelection.startLineNumber &&
...
function getSelectionWidgetPosition() {
    if (!editor || !monacoApi || !isActiveResponseReady())
        return null;
...
function ensureSelectionWidget() {
    if (!editor || !monacoApi || selectionWidget !== null)
        return;
...
function updateSelectionWidget() {
    if (!editor || selectionWidget === null)
        return;
    const target = getActiveCommentTarget();
...
function modeHintText(active, commentTarget) {
    if (!active)
        return 'Select a response to start reviewing.';
...
function renderSidebar() {
    ...
    `;
        ...
        visible.forEach(response => {
            ...
            const pending = state.pendingRequestIds[response.id] !== null &&
    ...
    const commentTarget = active !== null && isActiveResponseReady()
...
function saveCurrentScrollPosition() {
    if (!editor || !state.activeResponseId)
        return;
...
function getInlineComments(draft = activeDraft()) {
    if (!draft)
        return [];
    return draft.comments.filter(comment => comment.startLine !== null && comment.startLine !== undefined);
}
...
function findInlineCommentOverlappingRange(startLine, endLine, draft = activeDraft()) {
    ...
    return (getInlineComments(draft).find(comment => lineRangesOverlap(normalizedRange.startLine, normalizedRange.endLine, comment.startLine, inlineCommentLineEnd(comment))) || null);
}
...
function clearViewZones() {
    if (!editor || activeViewZones.length === 0)
        return;
...
function restoreEditorSelectionSnapshot(snapshot) {
    if (!editor || !monacoApi || !snapshot)
        return;
    const selection = new monacoApi.Selection(snapshot.startLineNumber, snapshot.startColumn, snapshot.endLineNumber, snapshot.endColumn);
    editor.focus();
...
function maybeOpenCommentForActiveEditorLine(options = {}) {
    ...
    if (!comment || comment.id === options.excludeCommentId)
        return;
    ...
    const flushPendingClipboardAction = (reason, options = {}) => {
        ...
        if (action.kind === 'write') {
            ...
            void sendHostRequest({
                ...
                if (pendingClipboardAction !== action)
                    return;
...
function autosizeCommentTextarea(textarea) {
    ...
    const minLines = 2;
    const maxLines = 8;
    const previousHeight = Number.parseFloat(textarea.style.height || '0') ||
            ...
            if (event.key === 'Escape') {
                ...
                if (textarea.value.trim().length > 0) {
                    ...
                    showConfirmModal({
                        ...
                        confirmTone: 'danger',
                        onConfirm: () => {
                            deleteComment(comment, comment.startLine !== null && comment.startLine !== undefined
...
function syncViewZones() {
    clearViewZones();
    if (!editor || !isActiveResponseReady())
        return;
    const draft = activeDraft();
    if (!draft)
        return;
    const inlineComments = draft.comments.filter(comment => comment.startLine !== null && comment.startLine !== undefined);
    inlineComments.forEach(comment => {
        ...
        const domNode = renderCommentDOM(comment, () => {
            ...
            if (!editor || zoneEntry.id === null || zoneEntry.zone === null)
                return;
...
function updateDecorations() {
    ...
    const inlineComments = draft
        ? draft.comments.filter(comment => comment.startLine !== null && comment.startLine !== undefined)
        : [];
...
function renderWholeResponseComments() {
    ...
    const comments = draft.comments.filter(comment => comment.startLine === null || comment.startLine === undefined);
...
function maybeInitializeEditorCursor() {
    if (hasInitializedEditorCursor || !editor || !model)
        return;
...
function buildSubmitPayload() {
    ...
    const draft = activeDraft();
    if (!draft || !state.activeResponseId)
        return null;
    const payload = {
        type: 'submit',
...
function clearPendingGutterClick() {
    if (pendingGutterClickTimer === null)
        return;
...
function createGlyphHoverActions() {
    editor.onMouseMove(event => {
        ...
        if (target.type === monacoApi.editor.MouseTargetType.GUTTER_GLYPH_MARGIN ||
    ...
    editor.onMouseDown(event => {
        ...
        if (clickCount >= 2 && line !== null && line !== undefined) {
            clearPendingGutterClick();
            const lineEndColumn = model !== null && model !== undefined
...
window.addEventListener('paste', event => {
    debugLog('window paste event', {
        hasClipboardData: event.clipboardData !== null && event.clipboardData !== undefined,
        activeTag: document.activeElement?.tagName ?? null,
    });
}, true);
    ...
    window.require(['vs/editor/editor.main'], () => {
        ...
        editor.onKeyDown(event => {
            if (!event.metaKey &&
                !event.ctrlKey &&
                !event.altKey &&
                [monacoApi.KeyCode.UpArrow, monacoApi.KeyCode.DownArrow].includes(event.keyCode)) {
            ...
            if (event.keyCode !== monacoApi.KeyCode.Enter ||
                event.metaKey ||
                event.ctrlKey ||
                event.altKey) {
                return;
        ...
        editor.onDidChangeCursorSelection(() => {
            if (normalizeEditorSelectionToWholeLines())
                return;
            const active = activeResponse();
            const commentTarget = active !== null && isActiveResponseReady()
                ? getActiveCommentTarget()
...
ensureActiveResponse();
if (state.activeResponseId)
    ensureResponseLoaded(state.activeResponseId);
renderSidebar();
renderWholeResponseComments();
updateSidebarLayout();
setupMonaco();
